...
function VnpayPaymentPage(props) {
  ...
  let handleOnclick = async () => {
    // Kiểm tra nếu nội dung thanh toán bị bỏ trống
    if (
      !inputValues.orderDescription ||
      inputValues.orderDescription.trim() === ""
    ) {
      toast.warning("Nội dung thanh toán không được để trống");
      return;
    }

    // Gửi yêu cầu thanh toán nếu tất cả thông tin hợp lệ
