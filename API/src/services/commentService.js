        ...
        try {
            ...
            if (reviews && reviews.length > 0) {
                for (let i = 0; i < reviews.length; i++) {
                    // Kiểm tra và xử lý hình ảnh cho từng bình luận
                    reviews[i].image = reviews[i].image ? Buffer.from(reviews[i].image, 'base64').toString('binary') : '';

                    // Lấy danh sách bình luận con
                    let childComments = await db.Comment.findAll({
                        where: { parentId: reviews[i].id },
                        raw: true
                    });
                    reviews[i].childComment = childComments;
