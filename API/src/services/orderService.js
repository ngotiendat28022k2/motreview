      ...
      if (!id) {
        ...
        for (let i = 0; i < orderDetail.length; i++) {
          orderDetail[i].productDetailSize = await db.ProductDetailSize.findOne(
            {
              where: { productdetailid: orderDetail[i].productId },
              include: [{ model: db.Allcode, as: "sizeData" }],
              raw: true,
              nest: true,
            }
          );
          console.log("orderDetail[i]", orderDetail[i]);
            ...
            for (let k = 0; k < orderDetail.length; k++) {
              orderDetail[k].productDetailSize =
                await db.ProductDetailSize.findOne({
                  where: { productdetailid: orderDetail[k].productId },
                  include: [{ model: db.Allcode, as: "sizeData" }],
                  raw: true,
                  nest: true,
                });

              console.log("orderDetail[k]", orderDetail[k]);
