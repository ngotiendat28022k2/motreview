...
let sendSimpleEmail = async (dataSend) => {
  ...
  if (dataSend.type === "verifyEmail") {
    let info = await transporter.sendMail({
      from: '"buscu 👻" <datmau20@gmail.com>', // sender address
      to: dataSend.email, // list of receivers
      subject: "Xác thực email | NIKISHOP", // Subject line
      html: getBodyHTMLEmailVerify(dataSend),
    });
  }
  if (dataSend.type === "forgotpassword") {
    let info = await transporter.sendMail({
      from: '"buscu 👻" <datmau20@gmail@gmail.com>', // sender address
      to: dataSend.email, // list of receivers
      subject: "Xác nhận quên mật khẩu | NIKISHOP", // Subject line
